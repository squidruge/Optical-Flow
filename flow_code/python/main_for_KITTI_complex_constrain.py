...
def flow_func(x, theta, DOP, Zd, phi, h):
    v = x[0]
    u = x[1]
    Xd = Zd * tan(phi) * DOP
...
def fv_func(x, theta, DOP, Zd, phi, h):
    v = x[0]
    u = x[1]
    Xd = Zd * tan(phi) * DOP
    lambda_1 = (u - u0) / fx * sin(theta) + (v - v0) / fy * cos(theta)
...
def fu_func(x, theta, DOP, Zd, phi, h):
    v = x[0]
    u = x[1]
    Xd = Zd * tan(phi) * DOP
    lambda_1 = (u - u0) / fx * sin(theta) + (v - v0) / fy * cos(theta)
...
popt, _ = curve_fit(flow_func, vu, flow_val,
                    p0=[0, 0.5, 0, 0, 1.5],
                    bounds=[[-0.5 * pi, 0, 0, -0.5 * pi, 1], [0.5 * pi, 1, 10, 0.5 * pi, 3]])
...
theta_est = popt[0]
Xd_est = popt[1] * popt[2] * tan(popt[3])
Zd_est = popt[2]
phi_est = popt[3]
h_est = popt[4]
...
with np.errstate(divide='ignore', invalid='ignore'):
    fu_error = fu_diff[:, 0:u_max] * mask[v_head:v_tail] / fu[v_head:v_tail, 0:u_max]
    fu_error[~ np.isfinite(fu_error)] = 0  # 对 -inf, inf, NaN进行修正，置为0

with np.errstate(divide='ignore', invalid='ignore'):
    fv_error = fv_diff[:, 0:u_max] * mask[v_head:v_tail] / fv[v_head:v_tail, 0:u_max]
